...
class GenerationConfig:
    ...
    def __post_init__(self):
        if self.effects is None:
            self.effects = ['gradient', 'bubbles', 'dots']
        if self.export_sizes is None:
            self.export_sizes = [(400, 400)]
    ...
    def add_curve_patterns(self, count: int = 8):
        ...
        for _ in range(count):
            ...
            if curve_type == 'bezier':
                p0 = np.array([random.randint(0, self.width), random.randint(0, self.height)])
                p1 = np.array([random.randint(0, self.width), random.randint(0, self.height)])
                p2 = np.array([random.randint(0, self.width), random.randint(0, self.height)])
                p3 = np.array([random.randint(0, self.width), random.randint(0, self.height)])
                points = curve_gen.bezier_curve(p0, p1, p2, p3, 50)
                for i in range(len(points) - 1):
                    if (0 <= points[i][0] < self.width and 0 <= points[i][1] < self.height and
                            0 <= points[i + 1][0] < self.width and 0 <= points[i + 1][1] < self.height):
                        self.draw.line([tuple(points[i]), tuple(points[i + 1])], fill=color, width=1)
                ...
                for i in range(len(points) - 1):
                    if (0 <= points[i][0] < self.width and 0 <= points[i][1] < self.height and
                            0 <= points[i + 1][0] < self.width and 0 <= points[i + 1][1] < self.height):
                        self.draw.line([points[i], points[i + 1]], fill=color, width=1)
...
class EffectsProcessor:
    ...
    def add_dynamic_noise(img_array: np.ndarray, noise_type: str = 'perlin') -> np.ndarray:
        ...
        if noise_type == 'perlin':
            # 简化的柏林噪声模拟
            noise = np.zeros(img_array.shape[:2], dtype=np.float32)  # 修复数据类型
            for octave in range(3):
                ...
                noise = noise + octave_noise.astype(np.float32)  # 确保类型一致
...
class EnhancedBackgroundGenerator:
    ...
    def create_background(self, config: GenerationConfig = None) -> Image.Image:
        ...
        base_img = Image.fromarray(gradient.astype(np.uint8), mode='RGB')
        ...
        if self.config.noise_intensity > 0:
            ...
            img_array = EffectsProcessor.add_dynamic_noise(img_array)
    ...
    def generate_random_configs(self, count: int) -> List[GenerationConfig]:
        ...
        effect_pools = [
            ['gradient', 'bubbles', 'dots'],
            ['watermark', 'particles', 'curves'],
            ['grid', 'shapes', 'lighting'],
            ['bubbles', 'particles', 'depth_blur'],
...
def create_random_background(width: int = 400, height: int = 400,
    ...
    config = GenerationConfig(
        ...
        color_scheme=random.choice(['pastel', 'vibrant', 'ocean', 'sunset', 'forest', 'random']),
...
def create_themed_background(theme: str = 'corporate', width: int = 400, height: int = 400) -> Image.Image:
    """创建主题背景"""
    themes = {
        'corporate': {
            'style': 'minimal',
            'color_scheme': 'monochrome',
            'effects': ['gradient', 'grid', 'dots'],
            'element_density': 0.6,
        ...
        'ocean': {
            'style': 'bubble',
            'color_scheme': 'ocean',
            'effects': ['bubbles', 'particles', 'depth_blur'],
            'element_density': 1.0,
        ...
        'sunset': {
            'style': 'geometric',
            'color_scheme': 'sunset',
            'effects': ['shapes', 'lighting', 'color_overlay'],
            'element_density': 0.8,
...
if __name__ == "__main__":
    ...
    # 示例1：生成单个背景
    print("\n1. 生成单个背景...")
    single_bg = create_random_background(800, 600, 'mixed')
    ...
    for theme in themes:
        themed_bg = create_themed_background(theme, 400, 400)
    ...
    generated_files = api.generate_batch(
        ...
        height=400,
        style='mixed',
        effects=['curves', 'particles', 'bubbles']
    )
